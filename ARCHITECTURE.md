...
| `embedding_service.py` | Default **Groq HTTP embeddings** (`nomic-embed-text-v1.5`, with fallback to `v1_5`); optional **local** path |
...
- **Default (`EMBEDDING_PROVIDER=groq`):** `POST https://api.groq.com/openai/v1/embeddings` with model **`nomic-embed-text-v1.5`** (official Groq examples); if that id is rejected, one retry uses **`nomic-embed-text-v1_5`**. Batched requests (chunk size capped in code).  
